...
document.addEventListener("DOMContentLoaded", function (event) {
  ...
  for (let film of films) {
    filmsContent += `
            <div class= "film">
              <h2> ${film.name} </h2>
              <div> Вселенная:${film.universe} </div>
              <div> Альтер эго:${film.alterego} </div>
              <div>Род деятельности: ${film.occupation} </div>
              <div> Друзья: ${film.friends} </div>
              <div> Суперсилы: ${film.superpowers} </div>
              <img src="${film.url}" alt="Film Poster" width="80%"> 
              <div class="form_item">
      <div class="rating">
  ...
  document.getElementById("filmsContainer").innerHTML = filmsContent;
});
...
function initRatings() {
  ...
  function initRatingVars(rating) {
    ...
    ratingActive = rating.querySelector(".rating_value");
  }
